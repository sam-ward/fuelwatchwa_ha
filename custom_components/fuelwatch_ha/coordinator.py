...
class FuelWatchCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        try:
            ...
            _LOGGER.debug(
                "FuelWatch matched %s today / %s tomorrow stations within %skm for %s using %s",
                today_result["count"],
                tomorrow_result["count"] if tomorrow_result else 0,
                self.radius,
                self.fuel_type,
                location_label,
            )
    ...
    def _process_stations(
        ...
        """Filter stations by radius and compute cheapest/average/most_expensive."""
        if not data:
            return None
